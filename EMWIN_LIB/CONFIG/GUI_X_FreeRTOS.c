...
{ 
  return ((unsigned int) xTaskGetCurrentTaskHandle());
}
...
void GUI_X_Log (const char *s) { printf("<<GUI_Log>>%s\n",s);}
void GUI_X_Warn (const char *s)  { printf("<<GUI_Warn>>%s\n",s);}
void GUI_X_ErrorOut(const char *s)  { printf("<<GUI_ErrorOut>>%s\n",s);}
